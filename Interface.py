...
def openGUI():
    ...
    def show(btn):
        ...
        isGraph = 1
        if option_network.get() == "Authors":
                if option_graph.get() == "Degree Distribution":
                    graph,no,need = GetAuthorDegreeDistribution(data.GetAuthorGraph())
                    
                elif option_graph.get() == "Publication Distribution":
                    graph = GetAuthorPublicationDistribution(data.GetAuthorGraph())
                    
                elif option_graph.get() == "Reputation Distribution":
                    graph,no,need = GetAuthorReputationDistribution(data.GetAuthorGraph())
                    
                elif option_graph.get() == "Reputation Degree":
                    graph = GetAuthorReputationDegree(data.GetAuthorGraph())
                    
                elif option_graph.get() == "Maximum Degree Change":
                    graph = GetAuthorMaximumDegreeChange(data.GetAuthorGraph())
